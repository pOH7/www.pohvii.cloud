...
import { Calendar, ArrowRight, Folder, Tag, TrendingUp, Sun, Moon } from "lucide-react"
...
export default function BlogPage() {
  ...
  const popularPosts = [
    { title: "Building Modern Web Apps", description: "Complete guide to React and Next.js" },
    { title: "JavaScript Fundamentals", description: "Master core JS concepts" },
    { title: "CSS Grid Mastery", description: "Advanced layout techniques" },
    { title: "TypeScript Advanced Tips", description: "Better code with TypeScript" },
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      ...
      <section className="w-full py-12 px-4 md:px-8 max-w-7xl mx-auto">
        <div className="mb-8">
          <h2 className="text-3xl font-bold mb-2 animate-fade-in-up">Featured Posts</h2>
          <p className="text-muted-foreground animate-fade-in-up stagger-1">Discover my latest and most popular articles</p>
        </div>
        
        <div className="grid md:grid-cols-2 gap-8 mb-16">
          {featuredPosts.map((post, index) => (
            <Card key={post.slug} className={`blog-card-hover group overflow-hidden animate-fade-in-up stagger-${index + 2}`}>
      ...
      <section className="w-full px-4 md:px-8 max-w-7xl mx-auto pb-16">
        <div className="grid lg:grid-cols-4 gap-8">
          ...
          <div className="lg:col-span-3">
            <div className="mb-8">
              <h2 className="text-3xl font-bold mb-2 animate-slide-in-up">Recent Posts</h2>
              <p className="text-muted-foreground animate-slide-in-up stagger-1">Latest articles and tutorials</p>
            </div>
            
            <div className="grid md:grid-cols-2 xl:grid-cols-3 gap-6 mb-8">
              {recentPosts.map((post, index) => (
                <Card key={post.slug} className={`blog-card-hover group overflow-hidden animate-slide-in-up stagger-${index + 2}`}>
                  <div className="aspect-video overflow-hidden">
                    <Image 
                      src={post.image} 
                      alt={post.title}
            ...
            <div className="text-center">
              <Button size="lg" className="flex items-center gap-2 mx-auto animate-pulse-subtle">
            ...
            <Card className="animate-slide-in-up stagger-2">
              ...
              <CardContent className="space-y-2">
                {categories.map((category) => (
                  <div key={category.name} className="flex items-center justify-between py-2 px-3 rounded hover:bg-muted transition-colors cursor-pointer">
