---
title: >-
  The Complete Guide to favicon.ico - Technical Implementation, SEO Impact, and
  Automation
description: >-
  Master favicon implementation with this comprehensive guide covering technical
  specs, SEO benefits, automation scripts, and modern web development best
  practices.
date: 2025-09-11
...
For Next.js projects using the App Router, place favicon files in the `public` directory and use the Metadata API in your root layout:
...
> **Note**: Next.js also supports [file-based metadata](https://nextjs.org/docs/app/api-reference/file-conventions/metadata/app-icons) where you can place `icon.png` or `apple-icon.png` directly in the `app/` directory. However, this approach doesn't include the `sizes` attribute, so the Metadata API is recommended for complete favicon implementation.
...
- **Clear Safari favicon cache (macOS)**: Remove the cache directory at `~/Library/Safari/Favicon Cache/`
  ```bash
