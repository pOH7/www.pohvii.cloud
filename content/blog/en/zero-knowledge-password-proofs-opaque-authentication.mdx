...
const keyPair1 = await keyGen.generateKeyPair(
  "mypassword123",
  "user@example.com"
);
const keyPair2 = await keyGen.generateKeyPair(
  "mypassword123",
  "user@example.com"
);
...
console.log(
  "^ This is why production systems need proper libraries like libsodium"
);
```
...
class SecurityLimitations {
  async demonstrateIssues() {
    ...
    const guessedKeyPair = await this.generateKeyPair(
      weakPassword,
      "alice@example.com"
    );
...
**Registration Phase**: Password + OPRF → encrypted envelope containing client private key
...
class OPAQUEOfflineAttackPrevention {
  attemptOfflineAttack(credentialRecord) {
    ...
    return {
      ...
      reason:
        "OPRF evaluation requires server's private OPRF key - offline attacks impossible",
      contrast:
        "Traditional hashing allows offline brute force with stolen hashes",
    };
  }
...
class CloudflareOpaque {
  ...
  async registerUser(email, password) {
    ...
    const registrationRequest =
      await this.opaque.createRegistrationRequest(password);
  ...
  async authenticateUser(email, password) {
    const credentialRequest =
      await this.opaque.createCredentialRequest(password);
  ...
  analyzeAttackScenarios() {
    return {
      ...
      userTargeting: {
        ...
        opaque: {
          userEnumeration:
            "Prevented - constant-time indistinguishable responses",
      ...
      networkAttacks: {
        ...
        opaque: {
          ...
          certificateCompromise:
            "OPRF messages only - no direct password exposure",
  ...
  getOpaqueSecurityGuarantees() {
    return {
      ...
      userEnumerationResistance: {
        property:
          "Server responses for existing/non-existing users are indistinguishable",
...
const OPAQUE_SECURITY_GUARANTEES = {
  ...
  "Offline Attack Resistance":
    "OPRF requires server interaction for any password guess",
  "User Enumeration Resistance":
    "Server responses are computationally indistinguishable",
  "Mutual Authentication":
    "Both client and server cryptographically prove their identity",
  "Forward Secrecy":
    "Past sessions remain secure even if long-term keys are compromised",
  "Export Key Security":
    "Additional application keys derived without exposing base secrets",
};
```
...
_Have questions about implementing zero-knowledge authentication? Found this guide helpful? Share your thoughts and experiences in the comments below._
